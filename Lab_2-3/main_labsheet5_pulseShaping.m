...
rcPulse = raisedCosinePulse(5*tb, tb, fs, 0.1);
...
% eyediagram(basebandSinc, 2*tb/ts); title('Eye diagram for Sinc signal');
...
for a = 0.02: 0.3: 1
    rcPulse = raisedCosinePulse(5*tb, tb, fs, a);
    [basebandrc, trc] = impulse2Baseband(impulseTrain, rcPulse, fs);
    titleGraph = strcat("\alpha = ", num2str(a));
    eyediagram(basebandrc, 2*tb/ts); title(titleGraph);
end
